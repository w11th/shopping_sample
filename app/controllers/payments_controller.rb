class PaymentsController < ApplicationController
  ...
  def generate_pay
    orders = current_user.orders.where(order_no: params[:order_nos])
  ...
  def do_payment
    if @payment.success?
      ...
      if payment_success?
        ...
        redirect_to success_payemnts_path
      else
        @payment.do_failed_payemnt!(params)
  ...
  def find_and_validate_payment_no
    ...
    unless @payment
      if payment_success?
        ...
        render text: '未找到您的定翻好，同时您的支付没有成功，请返回重新支付'
      end
    end
  end
  ...
  def build_is_request_from_alipay?(result_options)
    return false if request_option[:notify_id].blank?

    body = RestClient.get ENV['ALIPAY_URL'] + '?' + {
      services: 'notify_verify',
      partner: ENV['ALIPAY_PID'],
  ...
  def build_is_request_sign_valid?(result_options)
    ...
    options.extrct!('controller', 'action', 'format')
